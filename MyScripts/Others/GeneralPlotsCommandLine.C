...
void PhaseSpace(Double_t Mass1, Double_t Mass3, Double_t Mass4, std::string Title) {
  ...
  for (Double_t Mass2 = 0.1; Mass2 < (Mass1-Mass3-Mass4); Mass2 += 0.1) {
    ...
    h2->GetXaxis()->SetTitle("Neutrino-meson invariant mass [GeV^2]");
    h2->GetYaxis()->SetTitle("Neutrino-lepton invariant mass [GeV^2]");
...
static void show_usage(std::string name)
{
    std::cerr << "Usage: " << name << " <option(s)>"
              ...
              << "\t-m,--dalitz\t\t3-body N production Dalitz plots"
              << "\t-p,--prod\t\tN production BRs"
              << "\t-d,--dec\t\tN decay BRs"
              << std::endl;
}
...
int GeneralPlotsCommandLine(int argc, char* argv[]) {
