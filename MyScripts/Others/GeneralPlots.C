...
Double_t PhaseSpace2(Double_t motherMass, Double_t MN, Double_t otherMass) {

  Double_t phaseSpace = TMath::Power(motherMass*motherMass-MN*MN-otherMass*otherMass,2) - 4.*MN*MN*otherMass*otherMass;

  return phaseSpace;
}

Double_t PhaseSpaceFactor2(Double_t motherMass, Double_t MN, Double_t otherMass, Double_t phaseSpace) {

  Double_t factor = 0.;

  if(phaseSpace > 0.) {
    factor = ((motherMass*motherMass*(MN*MN + otherMass*otherMass)-TMath::Power(MN*MN-otherMass*otherMass,2))*TMath::Power(phaseSpace,0.5))/(otherMass*otherMass*TMath::Power(motherMass*motherMass-otherMass*otherMass,2));
  }

  return factor;
}

Double_t PhaseSpace3(Double_t motherMass, Double_t MN, Double_t otherMass) {
...
Double_t PhaseSpaceFactor3(Double_t motherMass, Double_t MN, Double_t otherMass, Double_t phaseSpace) {
...
Double_t Compute2BR(Double_t motherMass, Double_t otherMass, Double_t Factor) {

  Double_t brt = 0.;

  if (Factor > 0.) {
    if (motherMass == DMass && otherMass == Me)
      brt = De2BR*Factor;
    else if (motherMass == DMass && otherMass == Mmu)
      brt = Dm2BR*Factor;
    else if (motherMass == DSMass && otherMass == Me)
      brt = DSe2BR*Factor;
    else if (motherMass == DSMass && otherMass == Mmu)
      brt = DSm2BR*Factor;
    else if (motherMass == DSMass && otherMass == Mtau)
      brt = DSt2BR*Factor;
    else if (motherMass == Mtau && otherMass == Mpi)
      brt = TauPi2BR*Factor;
  }
...
Double_t Compute3BR(Double_t motherMass, Double_t otherMass, Double_t Factor) {
...
void GeneralPlots() {
  ...
  for (Int_t MN = 0; MN < Mass; MN += step) {
    PS = PhaseSpace2(DMass, MN, Me);
    PSF = PhaseSpaceFactor2(DMass, MN, Me, PS);
    BR = Compute2BR(DMass, Me, PSF);
  ...
  for (Int_t MN = 0; MN < Mass; MN += step) {
    PS = PhaseSpace2(DMass, MN, Mmu);
    PSF = PhaseSpaceFactor2(DMass, MN, Mmu, PS);
    BR = Compute2BR(DMass, Mmu, PSF);
    xMN[MN] = MN;
    if (PSF >= 0.)
      yPSF[MN] = PSF;
    yBR[MN] = BR;
  ...
  for (Int_t MN = 0; MN < Mass; MN += step) {
    PS = PhaseSpace2(DSMass, MN, Me);
    PSF = PhaseSpaceFactor2(DSMass, MN, Me, PS);
    BR = Compute2BR(DSMass, Me, PSF);
  ...
  for (Int_t MN = 0; MN < Mass; MN += step) {
    PS = PhaseSpace2(DSMass, MN, Mmu);
    PSF = PhaseSpaceFactor2(DSMass, MN, Mmu, PS);
    BR = Compute2BR(DSMass, Mmu, PSF);
    xMN[MN] = MN;
    if (PSF >= 0.)
      yPSF[MN] = PSF;
    yBR[MN] = BR;
