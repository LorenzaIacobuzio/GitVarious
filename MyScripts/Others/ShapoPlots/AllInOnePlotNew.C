...
void SetModel(Int_t model) {
  if (model == 1) {
    ...
    UmuSquared = USquared/11.;
    UeSquared = 0.;
    UtauSquared = 10.*UmuSquared;
  }
  else if (model == 4) {
    UmuSquared = USquared/11.;
    UeSquared = 10.*UmuSquared;
    UtauSquared = 0.;
  }
}
...
Double_t ThreeBodyBR(Double_t Mass1, Double_t Mass2, Double_t Mass3, Double_t Mass4) {
  ...
  if (Mass3 == K || Mass3 == K0 || Mass3 == pi || Mass3 == pi0) {
	...
	for (Double_t EN = ENmin; EN < ENmax; EN += ENstep) {
	  ...
	  TF2 *func = new TF2("func", "(([6]*[6]/2.)*(x - [2]*[2] - [4]*[4] + ([0] - 2.*[9]*[10])*([1] - x - ([0] - 2.*[9]*[10]))/([3]*[3])) + (([7]+[8]*1./x)*([7]+[8]*1./x)/2.)*([2]*[2] + [4]*[4])*(x - [2]*[2] + [4]*[4])*(([1] - x)*([1] - x)/(4.*[3]*[3]) - x) + 2.*[7]*[7]*[3]*[3]*(([1] - x)*([1] - x)/(4.*[3]*[3]) - x)*([2]*[2] + [4]*[4] - x + ([0] - 2.*[9]*[10])*([1] - x - ([0] - 2.*[9]*[10]))/([3]*[3])) + 2.*[7]*([7]+[8]*1./x)*([2]*[2]*([0] - 2.*[9]*[10]) + ([1] - x - ([0] - 2.*[9]*[10]))*[4]*[4])*(([1] - x)*([1] - x)/(4.*[3]*[3]) - x) + 2.*[5]*[6]*(x*(2.*([0] - 2.*[9]*[10]) - [1] + x) + ([1] - x)*([2]*[2] - [4]*[4])) + ([6]*([7]+[8]*1./x)/2.)*(([0] - 2.*[9]*[10])*([1] - x)/([3]*[3])*([2]*[2] - [4]*[4]) + ([1] - x)*([1] - x)*[4]*[4]/([3]*[3]) + 2.*TMath::Power([2]*[2] - [4]*[4], 2.) - 2.*x*([2]*[2] + [4]*[4])) + [6]*[7]*(([1] - x)*([0] - 2.*[9]*[10])*([1] - x - ([0] - 2.*[9]*[10]))/([3]*[3]) + 2.*([0] - 2.*[9]*[10])*([4]*[4] - [2]*[2]) + ([1] - x)*([2]*[2] - [4]*[4] - x)) + [5]*[5]*(([1] - x)*([1] - x)*(x - [2]*[2] + [4]*[4]) - 2.*[3]*[3]*(x*x - TMath::Power([2]*[2] - [4]*[4], 2.)) + 2.*([0] - 2.*[9]*[10])*([1] - x)*([2]*[2] - x - [4]*[4]) + 2.*([0] - 2.*[9]*[10])*([0] - 2.*[9]*[10])*x))");
...
void PhaseSpace(Double_t Mass1, Double_t Mass3, Double_t Mass4, std::string Title, Int_t model) {
  ...
  for (Double_t Mass2 = 0.5; Mass2 < 0.6; Mass2 += 0.1) {

    Int_t N = 100;
    Double_t xmin = (Mass4+Mass3)*(Mass4+Mass3);
    Double_t xmax = (Mass1-Mass2)*(Mass1-Mass2);
    ...
    if (title.Contains("D #rightarrow K^{0}eN")) {
      gPad->SaveAs("/home/li/cernbox/PhD/Thesis/Thesis/Thesis/images/dalitz.png");
      gPad->SaveAs("/home/li/cernbox/PhD/Thesis/Thesis/Thesis/images/dalitz.pdf");
    }
    else if (title.Contains("D^{0} #rightarrow K^{*}#muN")) {
      gPad->SaveAs("/home/li/cernbox/PhD/Thesis/Thesis/Thesis/images/dalitz1.png");
      gPad->SaveAs("/home/li/cernbox/PhD/Thesis/Thesis/Thesis/images/dalitz1.pdf");
    }
  }
}
...
void AllProd (Int_t model, TMultiGraph* M) {
  ...
  gPad->Update();
  gPad->Write();
  gPad->SaveAs(Form("/home/li/cernbox/PhD/Thesis/Thesis/Thesis/images/NSumProdGraph_%i.pdf", model));
  gPad->SaveAs(Form("/home/li/cernbox/PhD/Thesis/Thesis/Thesis/images/NSumProdGraph_%i.png", model));
}
...
void AllDecay(Int_t model, TMultiGraph* M) {
  ...
  M->SetMinimum(1.E-7);
  M->SetMaximum(1.5);
  M->GetXaxis()->SetLimits(0., Mass/1000.);
